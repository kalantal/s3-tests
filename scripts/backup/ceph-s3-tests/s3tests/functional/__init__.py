            ...
            for i in xrange(2):
                try:
                    try:
                        ...
                        if e.error_code not in ('NotImplemented', 'UnsupportedOperation'):
                            raise e
                        keys = bucket.list();
                    for key in keys:
...
def _make_request(method, bucket, key, body=None, authenticated=False, response_headers=None, request_headers=None, expires_in=100000, path_style=None, timeout=None):
    """
    ...
    if path_style is None:
        path_style = isinstance(bucket.connection.calling_format, OrdinaryCallingFormat)    
...
def _make_bucket_request(method, bucket, body=None, authenticated=False, response_headers=None, request_headers=None, expires_in=100000, path_style=None, timeout=None):
    """
