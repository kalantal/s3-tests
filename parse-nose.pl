...
foreach $testsuite ($tests->findnodes('/testsuite')) {
		...
		foreach $testpassed ($test->findnodes('./passed')) {
                        $failure_type = $testfailure->getAttribute('type');
                        $failure_temp = $testfailure->getAttribute('message');
                        }
...
$_ = $test_num-$test_errors-$test_failures-$test_skip;
print OUTFILE "\nTests = $test_num: Passed = $_, Errors = $test_errors, Failures = $test_failures, Skip = $test_skip \n";
